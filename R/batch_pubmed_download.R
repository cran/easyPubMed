    ...
    while (myRetstart < pubsNum){
      if (j < res_cn){
        ...
        while (is.null(myQuery) | is.null(tmpPapers)) {
          myQuery <- tryCatch(easyPubMed::get_pubmed_ids(pubmed_query_string), error = function(e) NULL)
          tmpPapers <- tryCatch(easyPubMed::fetch_pubmed_data(myQuery, retstart = myRetstart, retmax = myRetmax),
        ...
        doSaveXml <- tryCatch(XML::saveXML(tmpPapers, paste("xml_papers_", myExt, j, ".xml",sep="")), error = function(e) "ERROR")
        myQuery <- NULL
        tmpPapers <- NULL
        if (doSaveXml == "ERROR") {
